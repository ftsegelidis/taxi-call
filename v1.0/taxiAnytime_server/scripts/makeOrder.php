...
//$neworder->makeOrder( fixLatLon("34.2323"),fixLatLon("23432.23432"),fixLatLon("2345.2343"),fixLatLon("35.2325"),"/23453245345","/23425","800m"); 					  
...
public function makeOrder($customerlat,$customerlon,$driverlat,$driverlon,$customerdevice,$driverdevice,$distance){
	if(isset($customerlat) && isset($customerlon) && isset($driverlat) && isset($driverlon) && isset($customerdevice) && isset($driverdevice) && isset($distance) ){
		...
		$sql = "INSERT INTO orders
				...
				date = NOW(),
				taxidriverFK = (SELECT driverID FROM taxidriver WHERE driverDeviceID =  '".$this->driverid."' LIMIT 1),
				customerFK = (SELECT customerID FROM customer WHERE customerDeviceID =  '".$this->customerid."' LIMIT 1),
