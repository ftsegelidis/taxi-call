package app.taxiAnytimeDriver.pushService;
...
{
	// this is the log tag
	public static final String		TAG = "Taxi Anytime Push";
	

	// the IP address, where your MQTT broker is running.
	private static final String		MQTT_HOST = globalVariables.getInstance().getMQTT_IP_ADDRESS(); //209.124.50.174
	// the port at which the broker is running. 
	private static int				MQTT_BROKER_PORT_NUM   = globalVariables.getInstance().getMQTT_PORT();
	...
	public  static final double DISTANCE_LIMIT = 20000.0f; // Το εύρος σε (m) απ΄ όπου μπορεί να δέχεται ο οδηγός ειδοποιήσεις
	...
	// Notification title
	public static String			NOTIF_TITLE = "Taxi Anytime"; 	
	...
	/**
	 * @param text Το εισερχόμενο μύνημα από το mosquitto
	 * 
	 * @info
	 * Εμφανίζει το εισερχόμενο μύνημα ως ειδοποίηση στο χρήστη
	 * 
	 * @details
	 * Εμφανίζει το εισερχόμενο μύνημα ως ειδοποίηση στο χρήστη.
...
private void showNotification(String text) throws JSONException, InterruptedException, ExecutionException {
	...
	switch(type)
	{
		...
		case NEW_CUSTOMER:
		{
			...
			{
				distanceLimit = DISTANCE_LIMIT*2.0f; //σε περίπτωση που υπάρξει σφάλμα ορίζουμε μια απόσταση αυθαίρετα διπλάσια από την μέγιστη ,ώστε να είμαστε σίγουροι ότι δεν θα σταλεί ειδοποίηση
				e.printStackTrace();
			}
			
		//	Log.d("distance???",String.valueOf( distanceLimit) );
			
			if(distanceLimit <= DISTANCE_LIMIT && distanceLimit!= 0.0f ) // Αν η απόσταστη είναι μικρότερη από κάποια τιμή που ορίζουμε στο conf.java τότε να στείλει ειδοποίηση
			{
			
	
				Notification n = new Notification();
				n.flags |= Notification.FLAG_SHOW_LIGHTS;  // ΠΧ    kati |= 2 είναι ίδιο με  kati = kati | 2
				...
				Intent intent = new Intent(PushService.this, DriverActivity.class);
		...
		case ACCEPTED_CUSTOMER:
		{
			...
			SharedPreferences SendPrefs = this.getApplicationContext().getSharedPreferences(
					  "fromPush",Context.MODE_PRIVATE);
			SharedPreferences.Editor prefEditor = SendPrefs.edit();
			prefEditor.putString("customerAcceptLat",items[2]);
			prefEditor.putString("customerAcceptLon",items[3]);
			prefEditor.putString("customerAcceptedID",items[4]);
			prefEditor.putString("selectedOrderId",items[5]);
			...
			n.flags |= Notification.FLAG_AUTO_CANCEL;
			n.defaults = Notification.DEFAULT_ALL;
			n.icon = R.drawable.presence_online;
			n.when = System.currentTimeMillis();
		    ...
		    intent.setFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT);
			Bundle b = new Bundle();
			b.putString("customerAccept", items[1]);
			intent.putExtras(b);
	...
	protected class MQTTConnection implements MqttSimpleCallback {
		 ...
		 */
		/**
		 * @param topicName To θέμα στο οποίο γίνεται η αποστολή  (σε μας είναι της μορφής /id συσκευής)
		 * @param payload τα εισερχόμενα δεδομένα σε bytes μορφή
		 * 
		 * @info
		 * Δέχεται το εισερχόμενο μύνημα 
		 * 
		 * @details
		 * Δέχεται το εισερχόμενο μύνημα και το μετατρέπει σε string ώστε να σταλεί για ειδοποίηση
		 */
		public void publishArrived(String topicName, byte[] payload, int qos, boolean retained) throws JSONException, InterruptedException, ExecutionException {
			// Show a notification
